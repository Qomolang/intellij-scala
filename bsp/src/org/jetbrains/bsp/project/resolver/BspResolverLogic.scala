...
private[resolver] object BspResolverLogic {
  ...
  private[resolver] def calculateModuleDescriptions(buildTargets: collection.Seq[BuildTarget],
    ...
    val sharedSources = sharedSourceDirs(idToSources.mapValues(_.filterNot(_.generated)).toMap)
  ...
  private[resolver] def sharedModuleId(targets: collection.Seq[BuildTarget]): String = {
   ...
   */
  private[resolver] def createSyntheticModuleDescription(targets: collection.Seq[BuildTarget],
  ...
  private[resolver] def mergeModules(descriptions: collection.Seq[ModuleDescription]): ModuleDescription = {
    descriptions
      .sortBy(_.data.id)
  ...
  private[resolver] def calculateModuleDependencies(projectModules: ProjectModules): collection.Seq[ModuleDep] = for {
  ...
  private[resolver] def calculateSyntheticDependencies(moduleDependencies: collection.Seq[ModuleDep], projectModules: ProjectModules) = {
  ...
  private[resolver] def addModuleDependencies(dependencies: collection.Seq[ModuleDep],
